...
function handler(foo){
	if(foo.option=='recintent'){
		...
		update_date_recordatorio(foo.user._id, foo.recordatorio_id, foo.dateToSend);
	...
	db.select_many(busqueda,`recordatorios`, (err,res) => {
		if(!err){
			for (r of res){
				...
				fb.sendTextMessage(r.user_id, `⚠️RECORDATORIO⚠️\n"${r.message}"`);
			}
		}
	});
}
...
function save_recordatorio(user_id, message, dateToSend){
	...
	if(dateToSend > Date.now()){
		db.insert({user_id:user_id, message:message, dateToSend:dateToSend}, `recordatorios`, (err,res)=>{
			if(!err) fb.sendTextMessage(user_id, `Muy bien, te mandaré el mensaje: "${message}" en la siguiente fecha: "${format_date(dateToSend)}"`);
		})
	}else{
		fb.sendTextMessage(user_id, `Aún no me han implementado una función para viajar al pasado. 😒`);
	}
}
...
function delete_recordatorio(user_id, recordatorio_id){
	...
	db.delete({_id:recordatorio_id},`recordatorios`,(err)=>{
		if (err) return fb.sendTextMessage(user_id, `Tuve un problema al eliminar el recordatorio. Prueba más tarde.`);
...
function ask_new_date_recordatorio(user_id, recordatorio_id){
	recordatorio_id = new mongo.ObjectID(recordatorio_id);
	db.update({_id:user_id}, {changeDate:recordatorio_id}, `users`, (err,res)=>{
		if(err)return;
		fb.sendTextMessage(user_id, `¿Cuándo quieres que te mande el mensaje?`);
	});
}
...
function update_date_recordatorio(user_id, recordatorio_id, new_date){
	...
	if (!new_date) return fb.sendTextMessage(user_id, `No escribiste una fecha válida. Modificación cancelada.`);
	new_date = new Date(new_date);
	if(new_date < Date.now()) return fb.sendTextMessage(user_id, `Aún no me han implementado una función para viajar al pasado. 😒`);
	db.update({_id:recordatorio_id}, {dateToSend:new_date}, `recordatorios`, (err,res)=>{
		if (err) return fb.sendTextMessage(user_id, `Tuve un problema al actualizar la fecha del recordatorio. Prueba más tarde.`);
		fb.sendTextMessage(user_id, `Fecha modificada al ${format_date(new_date)}`);
	});
}

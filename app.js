    ...
    if (messageText) {
        ...
        if (quickReply) {
            if (/gato/i.test(quickReply.payload)) {
                ...
                recordatorios.handler({
                    option: quickReply.payload,
                    user: user,
                });
            }
        } else {
            if (user.changeDate) {
                ...
                recordatorios.handler({
                    option: "recnewdate",
                    user: user,
                    recordatorio_id: user.changeDate,
                    dateToSend: new_date,
                });
            } else if (nlp.intent) {
                ...
                switch (nlp.intent[0].value) {
                    ...
                    case "definir":
                        word = nlp.wikipedia_search_query
                            ? nlp.wikipedia_search_query[0].value
                            : nlp.math_expression
                            ? nlp.math_expression[0].value
                            : undefined;
                    ...
                    case "posponer":
                        ...
                        break;
                    case "calcular":
                        expresion = nlp.math_expression
                            ? nlp.math_expression[0].value
                            : undefined;
                        matEval.matEval(user._id, expresion);
                        break;
                    case "buscar":
                        query = nlp.wikipedia_search_query
                            ? nlp.wikipedia_search_query[0].value
                            : nlp.math_expression
                            ? nlp.math_expression[0].value
                            : undefined;
                ...
                fb.sendTextMessage(
                    user._id,
                    `Perdón, no te entendí. Si necesitas ayuda, solo escribe "ayuda".`
                );
            }
        }
        ...
        if (messageAttachments[0].type == "location") {
            ...
            } else if (user.useLocationFor == "weatherservice") {
                clima.subscribe_weather_service(user._id, lat, long);
            }
        }
    }
}
